...
let rec add_point_recd (p1: point_recd) (p2: point_recd) : point_recd =
...
let verify (enrollments : enrollment list) : bool =
  List.for_all (fun len -> List.length len = 1)
               (List.map (fun stdnt -> names (transcript enrollments stdnt))
                  (ids enrollments)) ;;
...
let rec partition (sort : 'a -> bool) (lst : 'a list) : ('a list * 'a list) =
  List.filter sort lst, List.filter (fun x -> not (sort x)) lst ;;
